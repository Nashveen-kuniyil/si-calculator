        ...
        <div className="bg-info p-5 rounded">
          ...
          <div className="mt-5">
            <form style={{ boxShadow: "4px 6px 8px black" }} className='border rounded  d-flex flex-column'>
              <TextField  style={{ backgroundColor: "whitesmoke" }}  id="principle"  name='principle'  value={principle}  onChange={(e) => handleValidation(e.target)}  label="Principle Amount"  variant="outlined" />
              {isPrincipleInputValid && <div className="text-danger">Invalid input</div>}

              <TextField  style={{ backgroundColor: "whitesmoke" }}  id="year" name='year'  value={year} onChange={(e) => handleValidation(e.target)} label="Year"  variant="filled" />
              {isYearInputValid && <div className="text-danger">Invalid input</div>}

              <TextField style={{ backgroundColor: "whitesmoke" }} id="rate"  name='rate'  value={rate}  onChange={(e) => handleValidation(e.target)}  label="Rate of Interest" variant="standard" />
